...
exports.getMunichWeather = getMunichWeather;
exports.getSpotifyNowPlaying = getSpotifyNowPlaying;
exports.sendMessage = sendMessage;
exports.sendTimerMessage = sendTimerMessage;
exports.sendTaskMessage = sendTaskMessage;
exports.getLettaStats = getLettaStats;
...
const apiQueue_1 = require("./apiQueue");
...
function formatToolCall(toolName, args) {
    ...
    if (toolName === 'discord_tool') {
        ...
        if (action === 'execute_batch' && args.operations) {
            ...
            const opNames = ops.map((op) => op.action || 'unknown').slice(0, 3);
    ...
    if (toolName === 'spotify_control') {
        ...
        if (action === 'execute_batch' && args.operations) {
            const ops = args.operations;
            const opActions = ops.map((op) => op.action || 'unknown').slice(0, 3);
            const more = ops.length > 3 ? ` +${ops.length - 3} more` : '';
            ...
            const req = https_1.default.request({
                ...
                res.on('end', () => {
                    try {
                        const json = JSON.parse(body);
                        if (json && json.item) {
                            console.log(`✅ Spotify: Now playing "${json.item.name}" by ${json.item.artists.map((a) => a.name).join(', ')}`);
                        }
                        else {
...
const processStream = async (response, discordTarget, discordClient, typingInterval, showReasoning = false, taskActionType, // 🔥 NEW: For channel_post tasks, don't redirect to heartbeat log
toolCalls, // 📝 For collecting tool calls
toolReturns, // 📝 For collecting tool returns
reasoningChain // 📝 For collecting reasoning chain (for reasoning models)
) => {
    ...
    const sendAsyncMessage = async (content, useHeartbeatLog = false) => {
        ...
        if (target && content.trim()) {
            ...
            if ('type' in target && target.type === 1) {
                isDM = true;
                channelName = 'DM';
            }
            try {
                ...
                (0, conversationLogger_1.logBotResponse)(content, channelId, channelName, undefined, // userId not available here
            ...
            if ('messageType' in chunk) {
                ...
                switch (chunk.messageType) {
                    ...
                    case 'reasoning_message':
                        ...
                        else if (typeof chunk.reasoning === 'string') {
                            reasoningText = chunk.reasoning;
                        }
                        ...
                        if (reasoningText && reasoningText.trim()) {
                            ...
                            if (showReasoning && discordTarget) {
                                ...
                                (0, conversationLogger_1.logReasoning)(reasoningText.trim(), channelId);
                            }
                            else {
                    ...
                    case 'tool_call_message':
                        ...
                        if ('toolCall' in chunk && chunk.toolCall) {
                            ...
                            if (discordTarget && 'name' in discordTarget && typeof discordTarget.name === 'string') {
                                channelName = discordTarget.name;
                            }
                            // Parse arguments
                            let parsedArgs;
                            try {
                            ...
                            // 📝 LOG: Tool call
                            (0, conversationLogger_1.logToolCall)(toolCall.name || 'unknown_tool', parsedArgs, channelId, channelName);
                            ...
                            if (toolCall.name === 'discord_tool' && parsedArgs) {
                                ...
                                if (parsedArgs.action === 'execute_batch' && parsedArgs.operations && Array.isArray(parsedArgs.operations)) {
                                    if (ALLOWED_DM_USER_ID) {
                                        const invalidOps = [];
                                        parsedArgs.operations.forEach((op, index) => {
                            ...
                            if (toolCall.name === 'discord_tool' && parsedArgs && taskActionType === 'user_reminder' && discordTarget) {
                                ...
                                // Handle execute_batch with send_message operations
                                    ...
                                    try {
                                        ...
                                        const sendMessageOps = parsedArgs.operations.filter((op) => op.action === 'send_message' && op.message);
                    ...
                    case 'tool_return_message':
                        ...
                        try {
                            if ('name' in chunk && typeof chunk.name === 'string') {
                                ...
                                try {
                                    (0, conversationLogger_1.logToolReturn)(chunk.name, returnValue, channelId, channelName);
                                }
                                catch (logError) {
...
`;
    ...
    try {
        ...
        const response = await apiQueue_1.apiQueue.enqueue(async () => {
            // 🔄 Wrap Letta API call with retry logic
            return await withRetry(async () => await client.agents.messages.createStream(AGENT_ID, {
        ...
        if (response) {
            try {
                ...
                (0, conversationLogger_1.logHeartbeat)(heartbeatContent, channelId, channelName);
...
async function sendMessage(discordMessageObject, messageType, conversationContext = null, customContent = null) {
    ...
    (0, conversationLogger_1.logLettaInput)(messageContent, // Full input including context
    conversationContext, [lettaMessage], // Message history (what we send to Letta)
    channelId, channelName, senderId, senderName, AGENT_ID);
    ...
    try {
        ...
        let response = null;
        try {
            response = await withRetry(async () => await client.agents.messages.createStream(AGENT_ID, {
                messages: [lettaMessage]
            }), MAX_API_RETRIES, // Controlled by env var (default: 1)
            'User message');
        }
            ...
            if (isServerError) {
                ...
                try {
                    ...
                    if (agentMessageResponse && agentMessageResponse.trim()) {
                        ...
                        (0, conversationLogger_1.logConversationTurn)(messageContent, agentMessageResponse, conversationContext, [lettaMessage], channelId, channelName, senderId, senderName, AGENT_ID);
                    }
                    return agentMessageResponse || "";
                }
                catch (fallbackError) {
        ...
        if (response) {
            try {
                agentMessageResponse = await processStream(response, discordMessageObject, undefined, typingInterval, false, undefined, toolCalls, // Pass by reference to collect tool calls
        ...
        if (agentMessageResponse && agentMessageResponse.trim()) {
            ...
            (0, conversationLogger_1.logBotResponse)(agentMessageResponse, channelId, channelName, senderId, senderName, undefined, // messageId not available here
            isDM);
            // 📝 LOG: Complete conversation turn (for fine-tuning) - Input + Output + Tools + Reasoning
            (0, conversationLogger_1.logConversationTurn)(messageContent, // Full input
            agentMessageResponse, // Full output
            conversationContext, [lettaMessage], // Message history
            channelId, channelName, senderId, senderName, AGENT_ID, toolCalls.length > 0 ? toolCalls : undefined, toolReturns.length > 0 ? toolReturns : undefined, reasoningChain.length > 0 ? reasoningChain : undefined);
...
async function sendTaskMessage(task, channel, discordClient) {
    if (!AGENT_ID) {
        console.error('Error: LETTA_AGENT_ID is not set');
        return SURFACE_ERRORS
            ? `Beep boop. My configuration is not set up properly. Please message me after I get fixed 👾`
    ...
    const isHeartbeatLogChannel = !!(channel && 'id' in channel && ((HEARTBEAT_LOG_CHANNEL_ID && channel.id === HEARTBEAT_LOG_CHANNEL_ID) ||
    ...
    // 📝 LOG: Task execution
    (0, conversationLogger_1.logTask)(taskName, actionType, taskContent, channelId, channelName);
    try {
        ...
        let response = null;
        try {
            response = await apiQueue_1.apiQueue.enqueue(async () => {
                // 🔄 Wrap Letta API call with retry logic
                return await withRetry(async () => await client.agents.messages.createStream(AGENT_ID, {
                    messages: [lettaMessage]
                }), MAX_API_RETRIES, // Controlled by env var (default: 1)
            ...
            if (isServerError) {
                ...
                try {
                    ...
                    if (taskResponse && taskResponse.trim()) {
                        (0, conversationLogger_1.logTaskResponse)(taskResponse, taskName, channelId, channelName);
                    }
                    return taskResponse || "";
                }
                catch (fallbackError) {
...
function getLettaStats() { return "Stats not implemented"; }
function getDailyStats() { return "Daily stats not implemented"; }
